...
% GM_window_button.mat and AGE.mat should be in the same folder.

load GM_window_button M G
...
lred = colorLab;
ASDg = 233;
ASDb = [212, 218, 20, 21, 240, 267, 269, 215, 270, 273 274 275 246 284 285];
...
meanAD = mean(AD);
sdAD = std(AD);
plot(age([NTg adultSetg]),Bmat([NTg adultSetg]),'o','linewidth',2,'markersize',10,'markerfacecolor',lred,'markeredgecolor',lred)
hold on
plot(age([NTb adultSetb]),Bmat([NTb adultSetb]),'o','linewidth',2,'markersize',10,'markerfacecolor',lblue,'markeredgecolor',lblue)
...
% [Fexp GoFexp] = fit(Age(Valid)',Bmat(Valid)',fcnexp,'start',[10000 .005 20]);
[Fexp GoFexp] = fit(Age(Valid)',Bmat(Valid)',fcnexp,'start',[100 .5 20]);
...
%ylim([0 0.35])
ylabel('Timing Error (ms)','fontweight','bold')
ylim([0 140])
...
csvwrite(sprintf('mat_pausing_lab_data_for_stat.csv'),stat)
